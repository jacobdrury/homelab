...
**OpenTofu CI:** GitHub Actions uses a **1Password Service Account** (`OP_SERVICE_ACCOUNT_TOKEN` in GitHub secrets only). Items: Cloudflare token, **Homelab R2 tofu state**, UniFi key, Tailscale OAuth (+ **Tailscale CI OAuth** for `tag:ci`), Uptime Kuma. Kuma API is Homelab NodePort via Connector (no CI kubeconfig). Setup: [opentofu-ci](../setup/opentofu-ci.md).
