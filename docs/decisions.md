...
| IaC | **GitOps first** (Helm/Argo/blueprints/ConfigMaps); **OpenTofu only** when no GitOps-native path (DNS, UniFi, Tailscale, Talos); secrets via 1Password — [iac](architecture/iac.md) · [agents](architecture/agents.md#gitops-first-opentofu-when-needed) |
...
| Postgres (apps) | **CloudNativePG** on `scarif-iscsi`. Per-app Clusters (`media-pg`, `authentik-pg`, HA Postgres) — **won't** collapse to one lab-wide Postgres. Chart-bundled Postgres only for demos |
...
| Host SSH / sudo | **1Password** + **SSH keys** (1Password agent); Host aliases (scarif, homelab02, arr archive, HA, pihole LXC until stopped, …); **shared lab admin sudo password** in 1P; no private keys in Git; Talos = **talosctl** |
...
| DNS app | **Pi-hole** in k8s — **Deployment ×3**, ConfigMaps SoT, Cilium L2 VIP `.22`; Authentik Proxy for UI — [pihole](architecture/pihole.md) |
| Pi-hole IaC | **ConfigMaps** in GitOps (`clusters/prd/apps/pihole/`); LXC OpenTofu retired |
...
| Apps (migrate order) | ~~*arr + qBit → Jellyfin → HA → Pi-hole~~ **done**. Discord bots **won't migrate**. **Homepage** = first GitOps app after Envoy |
...
| Transitional ingress | Envoy `*.lab` → in-cluster Services (media + HA + Pi-hole UI); scarif / proxmox still proxied — **no DNS/URL change** at cutover |
