...
| Homelab firewall | **Zone-Based Firewall** (UniFi OS 9+): Drury → Homelab allow all; **Homelab → IoT allow** (HA devices); Homelab → Guest/Camera deny; Homelab → Drury = Pi-hole DNS (+ transitional Envoy); IoT/Isolated → Pi-hole DNS — [networking](architecture/networking.md) |
